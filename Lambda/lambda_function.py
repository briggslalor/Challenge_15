...
def recommend_portfolio(intent_request):
    ...
    if parse_int(age) < 0 or parse_int(age) > 65:
        build_validation_result(False, age, "Your age is not valid.")
        
    if parse_int(investment_amount) < 5000:
        build_validation_result(False, investment_amount, "Your investment is too low.")
        
    if risk_level == "None":
        return("Your recommended portfolio is : 100% bonds (AGG), 0% equities (SPY).")
    elif risk_level == "Low":
        return("Your recommended portfolio is : 60% bonds (AGG), 40% equities (SPY).")
    elif risk_level == "Medium":
        return("Your recommended portfolio is : 40% bonds (AGG), 60% equities (SPY).")
    elif risk_level == "High":
        return("Your recommended portfolio is : 20% bonds (AGG), 80% equities (SPY).")
    else:
        return("You have not entered a valid risk level")
